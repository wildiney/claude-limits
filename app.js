...
class ClaudeTracker {
    ...
    updateHistory() {
        ...
        const historyIndex = this.settings.history.findIndex(h => h.date === today);
        if (historyIndex > -1) {
            this.settings.history[historyIndex].value = this.settings.usage;
        } else {
            this.settings.history.push({ date: today, value: this.settings.usage });
